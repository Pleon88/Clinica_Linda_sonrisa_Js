...
const show = async (req, res) => {
  ...
  const comunaFind = await Model.Comuna.findOne({
    where: { id_comuna: Number(usuarios.comuna_usuario) },
  });
...
const crearFromUser = async (req, res) => {
  const tipoUsuario = await Model.Tipo_Usuario.findAll();
...
const create = async (req, res) => {
  ...
  req.body.idTipo_usuario = req.body.idTipo_usuario
    ? req.body.idTipo_usuario
    : 5;
  req.body.status_usuario = req.body.status_usuario
    ? req.body.status_usuario
    : "Activo";
...
const createU = async (req, res) => {
  ...
  if (usuarios.message)
    return res.render(
      res.render("crear", { layout: "./Shared/layout_login", created: false })
    );
  // return res.status(200).json({usuarios})
  return res.render(
    res.render("crear", {
...
const edit = async (req, res) => {
  ...
  if (req.body.idTipo_usuario)
    req.body.idTipo_usuario = +req.body.idTipo_usuario;
  const usuario = await Model.Usuario.update(req.body, {
    where: { run_usuario: req.params.id },
  });
  if (usuario[0] === 1) {
    return res
      .status(200)
      .json({ error: false, message: "Usuario actualizado" });
  }
  return res
    .status(400)
    .json({ error: true, message: "Problemas al actualizar al usuario" });
  //return res.render('modificar', {layout: './Shared/layout_login', users: usuarios, user: req.user })
};

const remove = async (req, res) => {
  const usuario = await Model.Usuario.destroy({
    where: { run_usuario: req.params.id },
  });
  console.log(usuario);
  if (usuario === 1) {
